...
const getTestLabelClassesIdByDatasetId = gql`
  query getTestLabelClassesIdByDatasetId {
    labelClasses(where: { datasetId: "dataset 1" }) {
      id
    }
  }
`;
const getTestLabelClassQuery = gql`
  query getTestLabelClass($id: ID!) {
...
describe("LabelClass resolver test suite", () => {
  ...
  it("should query paginated labelClasses ignoring linked datasets", async () => {
    ...
    const queryResult = await client.query({
      query: gql`
        query getFirstTwoLabelClassesIdSkipOne {
