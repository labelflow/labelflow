...
const imagesLoader = async () => {
  const images = await Promise.all(
    [
      ...
      createImage(name, await (await fetch(url)).blob())
    )
  );
  return { images };
};
...
export const OneImage: Story = (_, { loaded: { images } }) => {
  return (
    <LabellingTool
...
export const ThreeImages: Story = (_, { loaded: { images } }) => {
  return (
    <LabellingTool images={images} image={images[1]} router={fakeRouter} />
  );
};
