...
CONFIG_SCHEMA = SchemaBuilder().add("task_id", int).add("server_url", str).build()
...
class cvat_rest_api_task_images(SourceExtractor):
    def _image_local_path(self, item_id):
        ...
        return osp.join(
            self._cache_dir, "task_{}_frame_{:06d}.jpg".format(task_id, int(item_id))
        )

    def _make_image_loader(self, item_id):
        return lazy_image(item_id, lambda item_id: self._image_loader(item_id, self))
    ...
    def _download_image(self, item_id):
        ...
        self._cvat_cli.tasks_frame(
            task_id=self._config.task_id,
            frame_ids=[item_id],
            outdir=self._cache_dir,
            quality="original",
        )
    ...
    def _connect(self):
        ...
        print(
            "Enter credentials for '%s' to read task data:" % (self._config.server_url)
        )
        username = input("User: ")
    ...
    def __init__(self, url):
        ...
        for entry in image_list:
            ...
            image = Image(
                data=self._make_image_loader(item_id),
                path=self._image_local_path(item_id),
                size=size,
            )
