...
if resume_epoch != nEpochs:
    ...
    for epoch in range(resume_epoch, nEpochs):
        ...
        for ii, sample_batched in enumerate(trainloader):
            ...
            # Compute the losses
            loss_coarse_outs1 = class_cross_entropy_loss(
                coarse_outs1, gts, void_pixels=void_pixels
            )
            loss_coarse_outs2 = class_cross_entropy_loss(
                coarse_outs2, gts, void_pixels=void_pixels
            )
            loss_coarse_outs3 = class_cross_entropy_loss(
                coarse_outs3, gts, void_pixels=void_pixels
            )
            loss_coarse_outs4 = class_cross_entropy_loss(
                coarse_outs4, gts, void_pixels=void_pixels
            )
            loss_fine_out = class_cross_entropy_loss(
                fine_out, gts, void_pixels=void_pixels
            )
            loss = (
                loss_coarse_outs1
                + loss_coarse_outs2
