...
def eval_one_result(
    ...
    def mAPr(per_cat, thresholds):
        ...
        for ii, th in enumerate(thresholds):
            ...
            for jj, categ in enumerate(per_cat.keys()):
                per_cat_recall[jj] = np.sum(np.array(per_cat[categ]) > th) / len(
                    per_cat[categ]
                )
    ...
    for i, sample in enumerate(loader):
        ...
        if not one_mask_per_image:
            ...
            filename = os.path.join(folder, sample["meta"]["image"][0] + ".png")
        mask = np.array(Image.open(filename)).astype(np.float32) / 255.0
        ...
        if use_void_pixels:
            non_void_pixels_crop = helpers.crop_from_bbox(
                np.logical_not(void_pixels), bb
            )
        gt_crop = helpers.crop_from_bbox(gt, bb)
        if use_void_pixels:
            eval_result["all_percent"][i] = np.sum(
                (gt_crop != mask_crop) & non_void_pixels_crop
            ) / np.sum(non_void_pixels_crop)
        else:
            eval_result["all_percent"][i] = (
                np.sum((gt_crop != mask_crop)) / mask_crop.size
            )
    ...
    eval_result["mAPr-vol"] = mAPr(
        eval_result["per_categ_jaccard"], np.linspace(0.1, 0.9, 9)
    )
