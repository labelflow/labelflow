...
def crop2fullmask(
    ...
    assert not (
        im is None and im_size is None
    ), "You have to provide an image or the image size  True"
    if im is None:
        im_si = im_size
    else:
        im_si = im.shape
    ...
    bbox_init = (bbox[0] + relax, bbox[1] + relax, bbox[2] - relax, bbox[3] - relax)
    ...
    if mask_relax:
        result[
            bbox_init[1] : bbox_init[3] + 1, bbox_init[0] : bbox_init[2] + 1
...
def overlay_mask(im, ma, colors=None, alpha=0.5):
    ...
    if colors is None:
        colors = (
            np.load(os.path.join(os.path.dirname(__file__), "pascal_map.npy")) / 255.0
        )
    else:
        colors = np.append([[0.0, 0.0, 0.0]], colors, axis=0)
    ...
    if ma.ndim == 2:
        fg = (
            im * alpha + np.ones(im.shape) * (1 - alpha) * colors[1, :3]
    ...
    contours = cv2.findContours(
        total_ma.copy().astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )[-2:]
...
def overlay_masks(im, masks, alpha=0.5):
    colors = np.load(os.path.join(os.path.dirname(__file__), "pascal_map.npy")) / 255.0
    ...
    for ma in masks:
        ma = ma.astype(np.bool)
        fg = (
            im * alpha + np.ones(im.shape) * (1 - alpha) * colors[i, :3]
        )  # np.array([0,0,255])/255.0
        i = i + 1
        ov[ma == 1] = fg[ma == 1]
        total_ma += ma
        ...
        # [-2:] is s trick to be compatible both with opencv 2 and 3
        contours = cv2.findContours(
            ma.copy().astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )[-2:]
        cv2.drawContours(ov, contours[0], -1, (0.0, 0.0, 0.0), 1)
        cv2.drawContours(
            ov_black, contours[0], -1, (255, 255, 255), -1
        )  # only draw a round
...
def crop_from_bbox(img, bbox, zero_pad=False):
    ...
    if zero_pad:
        ...
        crop = np.zeros(
            (bbox_valid[3] - bbox_valid[1] + 1, bbox_valid[2] - bbox_valid[0] + 1),
            dtype=img.dtype,
        )
        offsets = (-bbox_valid[0], -bbox_valid[1])
    ...
    img = np.squeeze(img)
    if img.ndim == 2:
        crop[inds[1] : inds[3] + 1, inds[0] : inds[2] + 1] = img[
            bbox_valid[1] : bbox_valid[3] + 1, bbox_valid[0] : bbox_valid[2] + 1
        ]
    else:
...
def fixed_resize(sample, resolution, flagval=None):
    ...
    if isinstance(resolution, int):
        ...
        tmp[np.argmax(sample.shape[:2])] = int(
            round(
                float(resolution) / np.min(sample.shape[:2]) * np.max(sample.shape[:2])
            )
        )
...
def crop_from_mask(img, mask, relax=0, zero_pad=False):
    if mask.shape[:2] != img.shape[:2]:
        mask = cv2.resize(
            mask, dsize=tuple(reversed(img.shape[:2])), interpolation=cv2.INTER_NEAREST
        )
...
def make_gt(img, labels, sigma=10, one_mask_per_point=False):
    ...
    if labels is None:
        ...
        if labels.ndim == 1:
            ...
            gt_1 = np.zeros(shape=(h, w), dtype=np.float64)
            gt_0 = np.maximum(
                gt_0, make_gaussian((h, w), center=labels[0, :], sigma=sigma)
            )
            ...
            for ii in range(1, labels.shape[0]):
                gt_1 = np.maximum(
                    gt_1, make_gaussian((h, w), center=labels[ii, :], sigma=sigma)
                )
            gt_0 = np.maximum(
                gt_0, make_gaussian((h, w), center=labels[0, :], sigma=sigma)
            )
