...
class VOCSegmentation(data.Dataset):
    ...
    def __init__(
        ...
        _mask_dir = os.path.join(
            _voc_root, "SegmentationObject"
        )  # each object each color
    ...
    def _check_integrity(self):
        ...
        if _md5c != self.MD5:
            print(
                " MD5({}) did not match MD5({}) expected for {}".format(
    ...
    def _check_preprocess(self):
        ...
        if not os.path.isfile(_obj_list_file):
            ...
            return list(np.sort([str(x) for x in self.obj_dict.keys()])) == list(
                np.sort(self.im_ids)
            )
    ...
    def _download(self):
        ...
        if self._check_integrity():
            ...
            def _progress(count, block_size, total_size):
                sys.stdout.write(
                    "\r>> %s %.1f%%"
                    % (_fpath, float(count * block_size) / float(total_size) * 100.0)
                )
                sys.stdout.flush()
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        # Read Image
        _img = np.array(Image.open(self.images[_im_ii]).convert("RGB")).astype(
            np.float32
        )  ###zsy open image imread
        ...
        return (
            _img,
            _target,
            _void_pixels.astype(np.float32),
            _other_classes.astype(np.float32),
            _other_same_class.astype(np.float32),
...
if __name__ == "__main__":
    ...
    for i, sample in enumerate(dataloader):
        ...
        overlay = helpers.overlay_mask(
            helpers.tens2image(sample["image"]) / 255.0,
