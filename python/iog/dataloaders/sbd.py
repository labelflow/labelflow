    ...
    def __init__(
        ...
        if self.area_thres != 0:
            self.obj_list_file = os.path.join(
                self.dataset_dir,
                "_".join(self.split)
                + "_instances_area_thres-"
                + str(area_thres)
                + ".txt",
            )
        else:
            self.obj_list_file = os.path.join(
                self.dataset_dir, "_".join(self.split) + "_instances" + ".txt"
            )
        ...
        if (not self._check_preprocess()) or preprocess:
            print(
                "Preprocessing SBD dataset, this will take long, but it will be done only once."
            )
            self._preprocess()
